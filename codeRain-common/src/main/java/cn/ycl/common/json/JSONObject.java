...
public class JSONObject extends LinkedHashMap<String, Object> {
    ...
    public static class JSONArray extends ArrayList<Object> {
        ...
        public String toString() {
            try {
                return cn.ycl.common.json.JSON.marshal(this);
            } catch (Exception e) {
    ...
    public JSONObject(final cn.ycl.common.json.JSONObject other) {
        super(other);
    }
     ...
     * @return 返回本对象。
     */
    public cn.ycl.common.json.JSONObject value(final String name, final Object value) {
        final int indexDot = name.indexOf('.');
        if (indexDot >= 0) {
     ...
     */
    public cn.ycl.common.json.JSONObject obj(final String name) {
        final Matcher matcher = arrayNamePattern.matcher(name);
        if (matcher.find()) {
            return endArray(matcher.group(1), matcher.group(2), new EndArrayCallback<cn.ycl.common.json.JSONObject>() {
                @Override
                public cn.ycl.common.json.JSONObject callback(JSONArray arr, int index) {
                    return objAt(arr, index);
                }
            });
        } else {
            cn.ycl.common.json.JSONObject obj = getObj(name);
            if (obj == null) {
                obj = new cn.ycl.common.json.JSONObject();
                put(name, obj);
            }
            return obj;
        }
     ...
     * @return 返回指定的对象字段。
     */
    public cn.ycl.common.json.JSONObject getObj(final String name) {
        return (cn.ycl.common.json.JSONObject) get(name);
    }
     ...
     * @return 返回本对象
     */
    public cn.ycl.common.json.JSONObject set(final String name, final Object value) {
        put(name, value);
        return this;
    }
    ...
    public <T> T asBean(Class<T> beanClass) {
        try {
            return cn.ycl.common.json.JSON.unmarshal(cn.ycl.common.json.JSON.marshal(this), beanClass);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private static Object transfer(final Object value) {
        if (!(value instanceof cn.ycl.common.json.JSONObject) && value instanceof Map) {
    ...
    private static cn.ycl.common.json.JSONObject toObj(final Map<String, Object> map) {
        final cn.ycl.common.json.JSONObject obj = new cn.ycl.common.json.JSONObject();
     ...
     */
    private static cn.ycl.common.json.JSONObject objAt(final JSONArray arr, int index) {
        expand(arr, index);
        if (arr.get(index) == null) {
            arr.set(index, new cn.ycl.common.json.JSONObject());
        }
        return (cn.ycl.common.json.JSONObject) arr.get(index);
    }
